...
check('all six sections present', await page.locator('.dash-section').count() === 6);
check('nav has six items', await page.locator('[data-nav]').count() === 6);
check('preview iframe points at a scene', /scenes\//.test(await page.locator('#preview').getAttribute('src')));

// section switching
for (const id of ['theme', 'branding', 'scenes', 'widgets', 'obs']) {
...
await page.evaluate(() => {
  const i = document.querySelector('[data-theme="accent"]');
  i.value = '#ff0066'; i.dispatchEvent(new Event('input', { bubbles: true }));
});
await page.waitForTimeout(700);
const themed = await (await fetch(`${BASE}/api/state`)).json();
check('accent change persists to server state', themed.theme.accent === '#ff0066', themed.theme.accent);
...
await page.click('[data-motion="reduced"]');
await page.waitForTimeout(500);
const st2 = await (await fetch(`${BASE}/api/state`)).json();
check('motion level persists', st2.theme.motion === 'reduced', st2.theme.motion);

// theme reset
await page.click('#theme-reset');
await page.waitForTimeout(700);
const st3 = await (await fetch(`${BASE}/api/state`)).json();
check('theme reset restores defaults', st3.theme.accent === '#8B4DFF' && st3.theme.motion === 'full', JSON.stringify(st3.theme));
